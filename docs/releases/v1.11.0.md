...
- At release time, launch preparation used `POST https://api.programmable.market/v1/custom-launches`. That creation
  transport is now historical read-only compatibility; current discovery starts at
  `https://programmable.market/.well-known/programmable.json` and submits to
